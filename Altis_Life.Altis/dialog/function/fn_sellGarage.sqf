...
if(life_cgar_inUse+(300) >= time) exitWith {closeDialog 0;hint format["Pojazd można sprzedawać co 5 minut: %1:%2",4 - floor ((time - life_cgar_inUse) / 60),59 - round (time - life_cgar_inUse - (floor ((time - life_cgar_inUse) / 60)) * 60)];};
...
_toLog = format ["0|%1 (%4) has %2 for %3 sold.", name player, _vehicleName, [_price] call life_fnc_numberText,getPlayerUID player];
