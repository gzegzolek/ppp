...
if(!([false,"skalpel",1] call life_fnc_handleInv)) exitWith {hint "Nie masz Skalpela"};
...
if(life_inv_kidney >= 2) exitWith {hint "Nie potrzebujesz tak wiele nerek!"};//if you already have 2 kidneys, then go sell them already, no stockpiling
if(!([true,"kidney",1] call life_fnc_handleInv)) exitWith {hint "Nie masz miejsca na nerke!"};//if no room for kidney, you cannot take their kidney, duh, waste not want not
