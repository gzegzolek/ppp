...
if (life_is_processing) exitWith {hint "Kolega jest zajety. Prosze nie przeszkadzac!";};    
...
if(vehicle player != player) exitwith {hint format["Nie mo¿na siedzieæ w samochodzie"];};
...
if ((count _inputItems) != (count _inputItemsNeeded)) exitWith 
	{diag_log format["B³ad"];life_is_processing = false;};
...
if (_productionAmount < 1) exitWith {
	hint "Nie masz wystarczajaca materialow";
...
if (_productionAmount < 1) exitWith {
	hint "Nie masz wystarczajaco duzo pieniedzy";
	life_is_processing = false;
};
...
_pgText ctrlSetText format["%2 (1%1)...","%","Prosze Czekac."];
...
while{true} do
{
	...
	_pgText ctrlSetText format["%3 (%1%2)...",round(_cP * 100),"%","Prosze Czekac."];
	if(_cP >= 1) exitWith {};
	if(player distance _processor > 10) exitWith {};
};

if(player distance _processor > 10) exitWith {hint "Jestes zbyt daleko przetwarzania!"; 5 cutText ["","PLAIN"]; life_is_processing = false;};
...
if (_inputItemsDeleted) then {
	...
	if (!([true,_outputItem,(_outputItemAmount * _productionAmount)] call life_fnc_handleInv)) then {
		hint format["Ups. Podczas produkcji coœ posz³o nie tak!"];
		diag_log format["ERROR: fn_processMultipleItems - new item(s) are not added"];
	};

	//format output
	_formatedProcessingInfo = "Pomyslnie Przetworzyles";
	...
	_formatedProcessingInfo = _formatedProcessingInfo + format["",((_inputItemsNeeded select ((count _inputItemsNeeded) - 1)) * _productionAmount),(_inputItems select ((count _inputItems) - 1)),_outputItem,(_outputItemAmount * _productionAmount)];
