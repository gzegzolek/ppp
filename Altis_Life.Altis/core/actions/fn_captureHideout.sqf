...
if(!isNull _group) then {
	...
	_cpRate = 0.0045;
	[[1,format["%1 i jego Gang: %2 próbują odzyskać lokalną kryjówke",name player,(group player) getVariable "gang_name"]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
} else {
	_cpRate = 0.0075;
};
...
while {true} do
{
	...
	if(life_interrupted) exitWith {life_interrupted = false; titleText["Akcja Anulowana","PLAIN"]; life_action_inUse = false;_hideout setVariable["inCapture",false,true];};
	life_action_inUse = false;

	titleText["Kryjowka przejeta","PLAIN"];
	_flagTexture = [
