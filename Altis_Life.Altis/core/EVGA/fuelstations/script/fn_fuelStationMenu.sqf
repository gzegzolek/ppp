...
if(_fuelInUse) exitWith { hint format["Ciężarówka z benzyną jest właśnie używana!"]; life_action_inUse = false; };
...
if((count _veh) < 1) exitWith { hint format["Brak pojazdu"]; life_action_inUse = false; };
...
if(_mode == "FUELTRUCK") then {
	_txtTitle ctrlSetText format["Dystrybutor Paliwa  (%1 Litry)",_truckFuel];
} else {
	_txtTitle ctrlSetText format["Stacja Paliwa"];
};
...

{
	...
	_owner = "";
	if(_x in life_vehicles) then { _owner = " (Właściciel)"; };
	...
	if((_x != life_fuelstation_fueltruck) && _type in life_fuelstation_cfgTrucks) then {
		...
		if(_diff > 1) then {
			_listVeh lbAdd format["%1 - Naczepa%2", _info select 3, _owner];
			_listVeh lbSetPicture [(lbSize _listVeh)-1,(_info select 2)];
			_listVeh lbSetData [(lbSize _listVeh)-1,str(_x)];
