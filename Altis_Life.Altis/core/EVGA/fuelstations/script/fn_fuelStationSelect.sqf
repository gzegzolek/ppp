...
if(isNil "_veh" || {isNull _veh}) exitWith { life_action_inUse = false; hint "Pojazd nie jest w pobliżu!"; };

life_fuelstation_lockVeh = [_veh,_isTrailer];

_txtVeh ctrlShow true;
if(!isNull life_fuelstation_fueltruck) then {
	_txtVeh ctrlSetText format["Cysterna"];
} else {
	_txtVeh ctrlSetText format["Dystrybutor Paliwa ($%1 Litrów)", life_fuelstation_price];
};
...
_txtFit ctrlSetText format["%1 / %2 Litrów", _cashFit, _fitFuel];
