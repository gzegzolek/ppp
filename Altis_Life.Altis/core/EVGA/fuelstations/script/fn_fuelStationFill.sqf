...
_txtStation = "Dystrybutor Paliwa";
...
if(_isTrailer == 1) then { _txtVehicle = _txtVehicle + " - Naczepa"; };
...
_pgText ctrlSetText format["%1 (0 / %2 Litrów)...",_upp,life_fuelstation_amount];
...
while{alive player && !life_interrupted && _cP < 1 && _away} do
{
	...
	_pgText ctrlSetText format["%1 (%2 / %3 Litrów)...",_upp, round(_cP * life_fuelstation_amount), life_fuelstation_amount];
...
if(life_interrupted || !alive player || !_away) then {	
	_txtTmp = format ["Wlewanie benzyny zakończone, obecnie wlałeś %1 litrów", _succAmount];
...
if(_isTrailer == 1) then {
	_fillTrailer = ((_veh getVariable["fueltruck_fuel",0]) + _succAmount) min ([(typeOf _veh) + "_PRZYCZEPA"] call EVGA_fnc_vehFuelCap);
