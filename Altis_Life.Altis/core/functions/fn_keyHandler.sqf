...
{	
	...
	//Restraining (Shift + R) // Robbing
	case 19:
	{
		if(_shift) then {_handled = true;};
		if(_shift && playerSide == west && !isNull cursorTarget && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && (side cursorTarget in [civilian,independent]) && alive cursorTarget && cursorTarget distance player < 8 && !(cursorTarget getVariable "Escorting") && !(cursorTarget getVariable "restrained") && speed cursorTarget < 1) then
		{
			[] call life_fnc_restrainAction;
		};

		if(_shift && playerSide == civilian && !isNull cursorTarget && cursorTarget isKindOf "Man" && isPlayer cursorTarget && alive cursorTarget && cursorTarget distance player < 4 && speed cursorTarget < 1 && side cursortarget == civilian) then
		{
	...
	case 6:
	{
		...
		{
			hint "Nie mozesz otworzyc menu gracza kiedy jestes skuty!";
		};
		if(_shift) then {_handled = true;};
		if (_shift) then
		{
	...
	case 2:
	{
		[] call life_fnc_wantedMenu1;
	};
	
	//surrender...
	case 15:
	{
		if(!_alt && !_ctrlKey && !_shift && !life_isdowned) then 
		{
	...
	hint composeText [ image "icons\unlock.paa", " Otworzyłeś Pojazd" ];
	...
	hint composeText [ image "icons\lock.paa", " Zamknąłeś Pojazd" ];
	...
	case 62:
	{
		...
		if(_alt) then {
				...
				titleText["ALT + F4 Nie pomoże ;)","PLAIN"];
				diag_log format ["Serwer: %1 użył ALT + F4 !",_player getVariable["realname",name _player]];
        };
	};
	...
	case 48: 
	{
		 if(_shift && playerSide == west) then {
				...
				[] execVM "core\warner\barriermenu.sqf";
			  };
