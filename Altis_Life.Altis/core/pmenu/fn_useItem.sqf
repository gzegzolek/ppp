...
if((lbCurSel 2005) == -1) exitWith {hint "Musisz najpierw wybrac przedmiot!";};
...
switch (true) do
{
	...
	case (_item == "redgull"):
	{
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
			...
			[] spawn
			{
				...
				titleText["Run Forest run!!!!!","PLAIN"];
	...
	{
		if(!isNull life_spikestrip) exitWith {hint "Posiadasz juz kolczatke"};
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
	...
	case (_item == "fuelF"):
	{
		if(vehicle player != player) exitWith {hint "Nie mozesz zatankowac pojazdu bedac w nim!"};
