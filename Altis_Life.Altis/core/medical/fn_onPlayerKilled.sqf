...
waitUntil {_Timer ctrlSetText format["Respawn mozliwy za: %1",[(_maxTime - time),"MM:SS.MS"] call BIS_fnc_secondsToString]; 
round(_maxTime - time) <= 0 || isNull _this || Life_request_timer};
 
if (Life_request_timer) then {
_maxTime = time + (life_respawn_timer * 720);
waitUntil {_Timer ctrlSetText format["Respawn mozliwy za: %1",[(_maxTime - time),"MM:SS.MS"] call BIS_fnc_secondsToString]; 
...
_Timer ctrlSetText "Teraz mozesz sie odrodzic";
};
