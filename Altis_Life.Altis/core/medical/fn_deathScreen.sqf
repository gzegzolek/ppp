...
waitUntil {
	...
	if (_nearby == 99999) then { _medicsNear ctrlSetText format["Najbliższy Medyk: Brak",_nearby]; }
	else { _medicsNear ctrlSetText format["Najbliższy Medyk: %1m",[(round _nearby)] call life_fnc_numberText]; };
	sleep 1;
