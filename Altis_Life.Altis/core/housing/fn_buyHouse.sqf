...
_toLog = format ["0|%1 (%3) has a house for %2 bought.", name player, [(_houseCfg select 0)] call life_fnc_numberText,getPlayerUID player];
...
if(_action) then {
	if(life_cash < (_houseCfg select 0)) exitWith {hint format [localize "STR_House_NotEnough"]};
	...
	life_cash = life_cash - (_houseCfg select 0);
